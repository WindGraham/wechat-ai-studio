...
## 规则 4：对称 Padding 保证内容区等宽（新增）

**`padding-left` 做列间距时，如果只加在右侧列上，`box-sizing: border-box` 会让右侧列的内容区比左侧窄。**
...
所有列平分间隙，每列同时加 `padding-left` 和 `padding-right`，确保内容区完全相等：
...
**gap = 左列 padding-right + 右列 padding-left，且每列左右 padding 之和须相等，否则内容区不等宽。**

---

## 一句话总结

> **总宽度 ≤ 92%，间隙用 padding-left，每行独立容器，绝不赌换行。对称 padding 保证内容区等宽。**
