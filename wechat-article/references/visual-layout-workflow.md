...
> **边界**：`layout-composer.html` 是本地设计草稿工具，不是可直接粘贴到微信公众号编辑器的最终 HTML。
...
> 拖拽器自身可以使用 `<style>`、JavaScript、`position: absolute` 等浏览器能力；这些只存在于本地工具中，最终公众号 HTML 仍必须遵守 `wechat-rules.md`。
